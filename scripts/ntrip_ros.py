...
class NTRIPRos(Node):
        ...
        if rtcm_message_package == _MAVROS_MSGS_NAME:
            if have_mavros_msgs:
                ...
                self.get_logger().fatal(
                    f'The requested RTCM package {rtcm_message_package} is a valid option, but we were unable to import it. Please make sure you have it installed')
            ...
            self.get_logger().fatal(
                f'The RTCM package {rtcm_message_package} is not a valid option. Please choose between the following packages {_MAVROS_MSGS_NAME}, {_RTCM_MSGS_NAME}')
    ...
    def init_client(self):
        ...
        ssl = self.get_parameter('ssl').value
        cert = self.get_parameter('cert').value
