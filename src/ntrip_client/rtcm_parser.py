    ...
    def parse(self, buffer):
        ...
        while index < len(combined_buffer):
            ...
            if combined_buffer[index] == _RTCM_3_2_PREAMBLE:
                ...
                if len(combined_buffer) <= index + 2:
                    self._logdebug(
                        f'Found beginning of RTCM packet at {index}, but there is not enough data in the buffer to find the message length')
                ...
                if index + message_length + 6 <= len(combined_buffer):
                    ...
                    if expected_checksum == actual_checksum:
                        self._logdebug(f'Found valid packet at {index} with length {message_length}')
                        ...
                        self._logwarn(f'Actual Checksum:     0x{actual_checksum:X}')
                else:
                    self._logdebug(f'Found beginning of RTCM packet at {index}, but there is not enough data in the buffer to extract it, caching')
                    self._caching_data = True
        ...
        if self._caching_data:
            ...
            if len(self._buffer) > _MAX_BUFFER_SIZE:
                self._logwarn(f"Too much data buffered, trimming to {_MAX_BUFFER_SIZE} bytes.")
